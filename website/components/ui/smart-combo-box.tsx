...
type BaseProps = {
  ...
  /** Direction of dropdown - 'down' (default) or 'up' */
  direction?: 'down' | 'up';
};
...
export function SmartCombobox({
  ...
  return (
    <div
      ...
      style={{ overflow: 'visible', position: 'relative', zIndex: open ? 10000 : 'auto' }}
    >
      {label && (
        <label
          htmlFor={inputId}
      ...
      <div
        className={classNames(
          "group flex min-h-10 w-full items-center gap-2 rounded-md border glass-panel px-2",
          "border-light-border dark:border-dark-border focus-within:ring-2 focus-within:ring-primary-light-blue focus-within:ring-offset-2",
      ...
      >
          ...
          <div className="flex flex-wrap items-center gap-1">
            {internalValue.map((id) => {
              ...
              return (
                <span
                  key={id}
                  className="inline-flex items-center gap-1 rounded bg-primary-light-blue/20 px-2 py-0.5 text-xs text-primary-dark-blue dark:text-primary-light-blue"
                >
        ...
        <input
          ...
          disabled={disabled}
          className={classNames(
            "peer flex-1 bg-transparent outline-none placeholder:text-light-text-secondary dark:placeholder:text-dark-text-secondary text-light-text-primary dark:text-dark-text-primary"
          )}
          value={query}
          onChange={(e) => {
        ...
        <div className="ml-auto flex items-center gap-1">
          {clearable && ((isMultiple && internalValue.length > 0) || (!isMultiple && singleValue)) && (
            <button
              ...
              className="rounded p-1 text-light-text-secondary dark:text-dark-text-secondary hover:bg-primary-light-blue/10 hover:text-primary-light-blue"
      ...
      <div role="region" aria-live="polite" className="sr-only">
       ...
       {open && (
        <div
          data-smart-combobox-dropdown="true"
          data-direction={direction}
          className={classNames(
            "smart-combobox-dropdown z-[9999999] rounded-md border border-light-border dark:border-dark-border bg-white/95 dark:bg-dark-card/95 backdrop-blur-md shadow-2xl",
        ...
        >
          {header ? (
            <div className="border-b border-light-border dark:border-dark-border p-2 text-xs text-light-text-secondary dark:text-dark-text-secondary">
              {header}
            </div>
          ) : null}
          ...
          <div
            ...
            className="max-h-[inherit] overflow-auto webkit-scrollbar"
            style={{ maxHeight: Math.min(maxHeight, 200) }}
            onScroll={(e) => {
          ...
          >
            ...
            {showCreate && (
              <div
                ...
                onClick={() => handleCreate(debouncedQuery)}
                        className={classNames(
                          "smart-combobox-option flex cursor-pointer items-center gap-2 px-2 text-light-text-primary dark:text-dark-text-primary",
                          activeIndex === 0
                            ? "bg-primary-light-blue/20 text-primary-dark-blue dark:text-primary-light-blue"
                            : "hover:bg-primary-light-blue/10"
                        )}
                style={{ height: itemHeight }}
              >
                <span className="inline-flex size-5 items-center justify-center rounded bg-primary-light-blue/20 text-primary-dark-blue dark:text-primary-light-blue">
                  +
                </span>
            ...
            {Array.from(groups.entries()).map(([group, items]) => {
              return (
                <div key={group} role="group" aria-label={group}>
                  {group !== "Other" && (
                    <div className="sticky top-0 z-10 glass-panel px-2 py-1 text-[10px] uppercase tracking-wider text-light-text-secondary dark:text-dark-text-secondary">
                      {group}
                    </div>
                  )}
                  {items.map((opt: Option) => {
                    ...
                    const defaultRow = (
                      <div
                        ...
                        className={classNames(
                          "smart-combobox-option flex cursor-pointer items-center justify-between gap-2 px-2",
                      ...
                      >
                        ...
                        <div className="ml-2 flex items-center gap-2">
                          {opt.meta ? (
                            <span className={classNames("text-xs", active ? "opacity-80" : "text-light-text-secondary dark:text-dark-text-secondary")}>
                              {opt.meta}
                            </span>
                          ) : null}
                          {selected && (
                    ...
                    return renderOption ? (
                      <div
                        ...
                        onClick={() => toggleOption(opt)}
                        className={classNames(
                          "smart-combobox-option flex cursor-pointer items-center px-2",
                          active
                            ? "bg-primary-light-blue/20 text-primary-dark-blue dark:text-primary-light-blue"
                            : "hover:bg-primary-light-blue/10"
                        )}
            ...
            {!loading && options.length === 0 && !showCreate && (
              <div className="px-3 py-6 text-center text-sm text-light-text-secondary dark:text-dark-text-secondary">
                {emptyState ?? "No results"}
              </div>
            )}
            {loading && (
              <div className="flex items-center justify-center gap-2 px-3 py-6 text-sm text-light-text-secondary dark:text-dark-text-secondary">
                <span className="relative inline-flex">
                  <span className="size-4 animate-ping rounded-full bg-primary-light-blue/50" />
                  <span className="absolute inset-0 size-4 rounded-full bg-primary-light-blue" />
                </span>
                Loading…
              </div>
            )}
          ...
          {footer ? (
            <div className="border-t border-light-border dark:border-dark-border p-2 text-xs text-light-text-secondary dark:text-dark-text-secondary">
              {footer}
            </div>
          ) : null}
        </div>
